...
include("../conn/dataconnection.php");
 //连接数据库
@$kind_id=$_GET['k_id'];
@$kn=mysql_query("select kind_name from kind where kind_id='$kind_id'");
@$kn_n=mysql_fetch_array($kn);
@$kind_name=$kn_n['kind_name'];
$acct_yr = $_GET['acct_yr'];    //获取参数
$start_mth = $_GET['start_mth'];    //获取参数
$end_mth = $_GET['end_mth'];    //获取参数
$choose = $_GET['graph'];    //获取参数
     //执行SQL, 获得销量值
$query_rs_prod = "SELECT DISTINCT(acct_mth) FROM summary WHERE acct_yr = '$acct_yr' and acct_mth between '$start_mth' and '$end_mth' and kind_id='$kind_id'";
...
$graph->title->Set(iconv("UTF-8","GB2312//IGNORE",$acct_yr."年".$start_mth."~".$end_mth."月".$kind_name."销售统计图"));
...
$linePlot->SetLegend(iconv("UTF-8","GB2312//IGNORE","销量"));                                                          //设置图例名称
$linePlot2->SetLegend("总金额");
$linePlot3->SetLegend("利润");
...
//设置标题
$graph->title->Set(iconv("UTF-8","GB2312//IGNORE",$acct_yr."年".$start_mth."~".$end_mth."月".$kind_name."商品销售统计图"));
...
$barPlot->SetLegend(iconv("UTF-8","GB2312//IGNORE","消费金额(单位:万元)"));
$barPlot2->SetLegend(iconv("UTF-8","GB2312//IGNORE","消费金额(单位:万元)"));
$barPlot3->SetLegend(iconv("UTF-8","GB2312//IGNORE","消费金额(单位:万元)"));
...
//设置标题
$graph->title->Set(iconv("UTF-8","GB2312//IGNORE",$acct_yr."年".$start_mth."~".$end_mth."月".$kind_name."商品销量统计图"));
$graph->title->SetFont(FF_SIMSUN,FS_BOLD);
//得到饼图对象
